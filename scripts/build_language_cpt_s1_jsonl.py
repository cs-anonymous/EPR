...
DEFAULT_PLAN = [
    ...
    CorpusPlan("knowledge_format", "knowledge_format.jsonl", 20.0),
]
...
def append_full_repeats(input_path: Path, output_handle, repeats: int) -> int:
    written = 0
    for _ in range(repeats):
        for line in iter_nonempty_lines(input_path):
            output_handle.write(line)
            written += 1
    return written


def append_selected_indices(input_path: Path, output_handle, indices: set[int]) -> int:
    if not indices:
        return 0
...
def build_dataset(source_dir: Path, output_path: Path, summary_path: Path, seed: int) -> dict:
    ...
    with temp_output.open("w", encoding="utf-8") as out_handle:
        for plan in DEFAULT_PLAN:
            ...
            input_lines = count_nonempty_lines(input_path)
            if input_lines == 0:
            ...
            written = 0
            written += append_full_repeats(input_path, out_handle, full_repeats)
            written += append_selected_indices(input_path, out_handle, sampled_indices)
...
def main() -> None:
    ...
    summary = build_dataset(args.source_dir, args.output, args.summary, args.seed)
