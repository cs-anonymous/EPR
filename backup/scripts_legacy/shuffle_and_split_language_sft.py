...
def main() -> None:
    ...
    parser.add_argument(
        "--input",
        type=Path,
        default=Path("backup/legacy_CoReS/language_sft_s2/sft_language_train.jsonl"),
    )
    parser.add_argument(
        "--shuffled-output",
        type=Path,
        default=Path("backup/legacy_CoReS/language_sft_s2_shuffled.jsonl"),
    )
    parser.add_argument(
        "--train-output",
        type=Path,
        default=Path("backup/legacy_CoReS/language_sft_s2_train.jsonl"),
    )
    parser.add_argument(
        "--val-output",
        type=Path,
        default=Path("backup/legacy_CoReS/language_sft_s2_val.jsonl"),
    )
    parser.add_argument(
        "--val-alias-output",
        type=Path,
        default=Path("backup/legacy_CoReS/language_sft_val_10k.jsonl"),
    )
